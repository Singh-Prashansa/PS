        ...
        <div class="container-fluid">
          <div class="navbar-menu-wrapper d-flex align-items-center justify-content-between">
            <ul class="navbar-nav navbar-nav-left">
              ...
              <li class="nav-item dropdown">
                <a class="nav-link count-indicator dropdown-toggle d-flex align-items-center justify-content-center" id="notificationDropdown" href="#" data-bs-toggle="dropdown">
                  ...
                  <?php
                  $sql = "SELECT * FROM notification n LEFT JOIN all_items a ON n.ID = a.ID WHERE n.UID = ?";
                  ...
                  if ($row > 0) {
                      while ($row = mysqli_fetch_assoc($result)) {
                          if ($row['UID'] == $_SESSION['user_id']) {
                              echo '<a class="dropdown-item preview-item" href="claim.php?ID=' . $row['ID'] . '&NID=' . $row['NID'] . '">
                                      ...
                                      <div class="preview-item-content">
                                        <h6 class="preview-subject font-weight-normal">New ' . $row['item'] . ' found</h6>
                                        <p class="font-weight-light small-text mb-0 text-muted">Is this your belonging?</p>
                                      </div>
                                    </a>';
                          }
                      }
            ...
            <!-- <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
              <a class="navbar-brand brand-logo" href="../../index.html"><img src="../../images/1.png" width="200px" alt="logo" /></a>
              <a class="navbar-brand brand-logo-mini" href="../../index.html"><img src="../../images/2.png" alt="logo" /></a>
            </div> -->
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
              <div class="card-body">
                <h4 class="card-title">All related items</h4>
                <div class="table-responsive">
                  <?php 
                  
                  $sql = "SELECT * FROM `all_items`as a WHERE collected = ? AND a.item IN (select Item from `item_lost` where UID=? ) or a.description IN(select l.Description from `item_lost`as l where UID=?)ORDER BY ID DESC;";
                  $stmt = $conn->prepare($sql);
                  $collected=0;
                  $stmt->bind_param("iii", $collected,$_SESSION['user_id'],$_SESSION['user_id']);
                  $stmt->execute();
                  $result = $stmt->get_result();
                  if (!$result) {
                      die("Query failed: " . $stmt->error);
                    ...
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                          ...
                          echo '<td><a class="text-warning" href="claim.php?ID=' . $row['ID'] . '" style="text-decoration:none;">Claim</a></td>';
                          echo '</tr>';
                        }
                        ?>
                      </tbody>
