...
$sql = "SELECT a.* FROM all_items a WHERE a.collected = 0 ORDER BY a.ID DESC;";
...
if (isset($_POST['srh-btn'])) {
  ...
  if (!empty($from_date) && !empty($to_date)) {
      ...
      $sql = "SELECT * FROM `all_items` WHERE collected=0 AND DATE(date) BETWEEN ? AND ? ORDER BY ID DESC";

      $stmt = $conn->prepare($sql);
      $stmt->bind_param("ss", $from_date, $to_date);
  } else {
      $sql = "SELECT * FROM `all_items` where collected=0 ORDER BY ID DESC";
      $stmt = $conn->prepare($sql);
  }
} else {
    $sql = "SELECT * FROM `all_items` where collected=0 ORDER BY ID DESC";
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#export').click(function () {
                ...
                doc.text('Items found', 20, 25);

                var columns = [
                    { header: 'SN', dataKey: 'header1' },
                    { header: 'Item ID', dataKey: 'header2' },
                    { header: 'Item', dataKey: 'header3' },
                    { header: 'Description', dataKey: 'header4' },
                    { header: 'Date found', dataKey: 'header5' },
                    { header: 'Found location', dataKey: 'header6' },
