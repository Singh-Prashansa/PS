                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                if (isset($_GET['ID']) && isset($_GET['UID'])) {
                                    ...
                                    echo "Item ID or UserID not provided in the URL.";
                                }
                               
                                $sql = "SELECT a.ID, a.image, a.location, a.date, uc.ItemID, uc.item, uc.description, uc.date_claimed, r.fname, r.lname, r.UID, uc.date_found
                                        FROM user_claim uc 
                                        JOIN all_items a ON uc.ItemID = a.ID
                                        JOIN register r ON uc.UID = r.UID 
                                        WHERE uc.ItemID = ? AND r.UID = ?";
                                    ...
                                    <div class="table-responsive">
                                        <table class="table">
                                            <form action="" method="post">
                                                ...
                                                <tr>
                                                    <th>Collected by</th>
                                                    <td><input class="form-control form-control-lg border-light" type="text" name="collected_by" td></td>
                                                </tr>
                                                <tr>
                                                    <th>Select item</th>
                                                    <td>
                                                    <select class='form-control form-control-lg border-light text-dark ' name='Lost_ID'>
                                                    <option selected>Select item</option>
                                                    <?php

                                                    // Fetch additional data based on UID
                                                        $sql1 = "SELECT Lost_ID, Item FROM item_lost WHERE UID = ?";
                                                        ...
                                                        while ($row1 = mysqli_fetch_assoc($result1)) {
                                                            echo "
                                                                    <option value='" . $row1['Lost_ID'] . "'>" . $row1['Item'] . "</option>";
...
if(isset($_POST['check'])) {
    ...
    $Lost_ID = FormSanitizer::sanitizeFormForTags($_POST["Lost_ID"]);
    $code = FormSanitizer::sanitizeFormForTags($_POST["code"]);
    $collected_by = FormSanitizer::sanitizeFormName($_POST["collected_by"]);

    // to check code
    $sql = "SELECT Code FROM item_lost where Lost_ID=?;";
    $stmt = $conn->prepare($sql);
    ...
    if ($result->num_rows == 1) {
        ...
        if ($dbCode == $code) {
                    // update lost_item table to delete/lost_item 
                    // delete report after the item is collected
                    $stmt1 = $conn->prepare("DELETE FROM item_lost WHERE Lost_ID= ?");
                    $stmt1->bind_param("i", $Lost_ID);
                    $url = 'collected.php?ID=' . $ID . '&collected_by=' . urlencode($collected_by) . '&UID=' . $UID;
                    if ($stmt1->execute()) {
        ...
        exit();
                        ...
                        echo "Error updating item lost table: " . $stmt1->error;
                    }
        ...
        echo "<script>alert('Invalid Lost ID');</script>";
    }
